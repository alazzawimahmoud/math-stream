...
export function ResultsTable({ computation }: ResultsTableProps) {
  return (
    <Card className="bg-card border-border shadow-xl shadow-black/20 overflow-hidden border-t-4 border-t-secondary h-full flex flex-col w-full">
      <CardHeader className="bg-muted border-b border-border/50 h-[70px] py-2 px-4">
        <div className="flex items-center justify-between w-full h-full">
          <div className="space-y-0.5">
            ...
            <div className="text-[9px] text-foreground/40 font-black uppercase  flex items-center gap-2">
      ...
      <CardContent className="pt-4 sm:pt-6 flex-1 overflow-auto">
        {/* Total Progress */}
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-muted/50 rounded-xl border border-border/50 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-[9px] font-black uppercase  text-foreground/40">Total Execution Payload</span>
            <span className="text-sm font-black text-secondary font-mono ">{computation.totalProgress}%</span>
          </div>
          <Progress 
          ...
          <Table className="min-w-[600px] table-fixed">
            <TableHeader>
              <TableRow className="bg-muted border-border/50 hover:bg-muted transition-none">
                <TableHead className="text-[9px] font-black uppercase  text-foreground/40 py-2 px-2 sm:px-4 w-[100px]">Operation</TableHead>
                <TableHead className="text-[9px] font-black uppercase  text-foreground/40 py-2 px-2 sm:px-4 w-[180px]">Pipeline</TableHead>
                <TableHead className="text-[9px] font-black uppercase  text-foreground/40 py-2 px-2 sm:px-4">Computed Value</TableHead>
                <TableHead className="text-[9px] font-black uppercase  text-foreground/40 text-right py-2 px-2 sm:px-4 w-[120px]">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {computation.results.map((result) => {
                ...
                return (
                  <TableRow key={result.operation} className="border-border/50 hover:bg-card transition-all group">
                    <TableCell className="text-foreground font-black uppercase  text-xs py-2.5 sm:py-3.5 px-2 sm:px-4">
                    ...
                    <TableCell className="py-2.5 sm:py-3.5 px-2 sm:px-4">
                    ...
                    <TableCell className="text-foreground font-mono font-black text-sm sm:text-base py-2.5 sm:py-3.5 px-2 sm:px-4 max-w-0">
                    ...
                    <TableCell className="text-right py-2.5 sm:py-3.5 px-2 sm:px-4">
                      <Badge 
                        variant="secondary" 
                        className={`${status.bgColor} ${status.color} border-transparent font-black uppercase  text-[8px] px-1.5 sm:px-2 py-1 shadow-sm`}
                      >
